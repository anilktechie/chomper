...
from chomper.processors import *


class DefaultsTest(unittest.TestCase):

    def test_set_default(self):
        node = Defaults('defaults').value('Jeff')
        node.push = MagicMock()
        node(None)
        self.assertEqual(node.push.call_count, 1)
        self.assertEqual(node.push.call_args[0][0], 'Jeff')

    def test_set_dict_defaults(self):
        node = Defaults('defaults').values(dict(name='Annie', school='Greendale'))
        node.push = MagicMock()
        node(dict(name='Jeff'))

        args, kwargs = node.push.call_args
        self.assertEqual(node.push.call_count, 1)
        self.assertEqual(args[0]['name'], 'Jeff')
        self.assertEqual(args[0]['school'], 'Greendale')

    def test_set_dict_defaults_deep(self):
        defaults = dict(school=dict(name='Greendale', dean='Dean Pelton'))
        item = dict(school=dict(name='City College'))

        node = Defaults('defaults').values(defaults).deep()
        node.push = MagicMock()
        node(item)

        args, kwargs = node.push.call_args
        self.assertEqual(node.push.call_count, 1)
        self.assertEqual(args[0]['school']['name'], 'City College')
        self.assertEqual(args[0]['school']['dean'], 'Dean Pelton')
...
class SetTest(unittest.TestCase):
    ...
    def test_set_item(self):
        value = object()
        node = Set('set').value(value)
        node.push = MagicMock()
        node(None)
        self.assertEqual(node.push.call_count, 1)
        self.assertEqual(node.push.call_args[0][0], value)
